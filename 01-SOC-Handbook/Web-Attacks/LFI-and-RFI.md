...
*   **What:** LFI = Read local files; RFI = Execute remote files.
*   **Why:** Unsafe use of `include()`/`require()` with user-controlled paths.
*   **Impact:** LFI = Sensitive Config/Password leak. RFI = RCE (Remote Shell).
*   **Detection:** Traversal (`../`), System paths (`/etc/passwd`), Remote Schemes (`http://`) in URL params.
*   **Response:** Block IP → Check if outbound connection occurred (for RFI) → Check for "Web Shell" creation.
*   **Fix:** Disable `allow_url_include` (PHP), Whitelist allowed filenames, use indirect ID maps.

### 🎯 MITRE ATT&CK Mapping
*   **T1190:** Exploit Public-Facing Application.
*   **T1005:** Data from Local System (LFI).
*   **T1204:** User Execution (if RFI relies on user interaction, though usually automated).

### ⚠️ Avoid Assumptions
*   **LFI is "Read Only":** False. If an attacker can upload a file (avatar) and `include` it via LFI, they get RCE.
*   **RFI works everywhere:** RFI is rare in modern PHP because `allow_url_include` is OFF by default. LFI is much more common.
*   **Only PHP is vulnerable:** While most common in PHP, similar inclusion issues exist in Java, Node.js, and Python (Template Injection).
